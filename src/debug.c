...
static int invokeInstruction(const char *name, Chunk *chunk, int offset) {
  ...
  // Print the instruction name, the constant index operand, the argument count
  // operand, and the value of the constant at the index
  printf("%-16s (%d args) %4d '", name, argCount, constant);
  printValue(chunk->constants.values[constant]);
  printf("\n");

  // Return offset by 3 (opcode + constant operand + argument count operand)
  return offset + 3;
}
